...
class PDDL(Puzzle):
        ...
        for action in self.domain.actions:
            ...
            for param_combo in param_combinations:
                ...
                grounded_action = {
                    ...
                    'preconditions': self._ground_formula(action.precondition, param_combo, param_names),
    ...
    def _ground_formula(self, formula, param_substitution: List[str], param_names: List[str]) -> List[str]:
        ...
        if hasattr(formula, 'operands'):
            ...
            for operand in formula.operands:
                grounded_parts.extend(self._ground_formula(operand, param_substitution, param_names))
            return grounded_parts
        
        return []

    def _ground_effects(self, effect, param_substitution: List[str], param_names: List[str]) -> Tuple[List[str], List[str]]:
        ...
        # Handle And/Or effects
            ...
            for operand in effect.operands:
                part_add, part_delete = self._ground_effects(operand, param_substitution, param_names)
                add_effects.extend(part_add)
                delete_effects.extend(part_delete)
    ...
    def get_neighbours(
        ...
        s_next = jnp.logical_or(
            jnp.logical_and(s[None, :], ~self.del_mask),
            self.add_mask
        )
