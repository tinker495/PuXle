...
class DomainValidator:
    ...
    def validate(self, domain: Domain) -> Tuple[bool, List[str]]:
        ...
        for action in domain.actions:
            action_preds = self._extract_predicates(action.precondition) + \
            ...
            for pred in action_preds:
                if pred.name not in defined_predicates:
                    ...
                    errors.append(f"Action '{action.name}' uses predicate '{pred.name}' with arity {pred.arity}, expected {defined_predicates[pred.name]}")
