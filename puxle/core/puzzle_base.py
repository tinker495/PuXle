...
class Puzzle(ABC):
    ...
    def inverse_action_permutation(self) -> Optional[jnp.ndarray]:
        """Return inverse-neighbour permutation indices for reversible puzzles."""
        return self._inverse_action_permutation
    ...
    def _get_shuffled_state(
        ...
        Returns:
            A scrambled ``State``.
        """
        import puxle.core.scramble as scramble

        return scramble._get_shuffled_state(
            self, solve_config, init_state, key, num_shuffle
        )
    ...
    def batched_get_random_trajectory(
        ...
        return scramble._batched_get_random_trajectory(
            self, k_max, shuffle_parallel, key, non_backtracking_steps
        )
    ...
    def batched_get_random_inverse_trajectory(
        ...
        return scramble._batched_get_random_inverse_trajectory(
            self, k_max, shuffle_parallel, key, non_backtracking_steps
        )
