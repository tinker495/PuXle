...
class TestPDDLParametric:
    ...
    def test_parse_and_ground_counts_param(self, spec):
        ...
        assert puzzle.num_actions == spec.expected_actions, f"Expected {spec.expected_actions} actions, got {puzzle.num_actions}"
        assert puzzle.action_size == spec.expected_actions, f"Expected action_size {spec.expected_actions}, got {puzzle.action_size}"
    ...
    def test_neighbours_and_filled_contract_param(self, spec):
        ...
        if not puzzle.is_solved(solve_config, initial_state) and spec.solvable:
            assert jnp.any(jnp.isfinite(costs)), f"Solvable domain {spec.name} should have at least one applicable action"
    ...
    def test_solution_path_param(self, spec):
        ...
        if spec.solvable:
            ...
            assert solved or current_state != initial_state, f"Solvable domain {spec.name} should either reach goal or make progress"
        else:
