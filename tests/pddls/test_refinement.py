...
def test_iterative_fusion_structure(tmp_path):
    ...
    d1_path.write_text("(define (domain d1) (:requirements :strips) (:predicates (p)) (:action a1 :parameters () :precondition (p) :effect ()))")
...
def test_type_consistency_validator(tmp_path):
    ...
    # Case 1: Valid
    d_str = "(define (domain test) (:requirements :typing) (:types a) (:action a1 :parameters (?x - a) :precondition () :effect ()))"
    ...
    #     d2 = Domain(d1.name, requirements=d1.requirements, types=d1.types, predicates=d1.predicates, actions=[a_invalid])
    #     valid, errors = validator.validate(d2)
    #     assert not valid
    # except Exception:
...
def test_varying_depth_benchmark(tmp_path):
    ...
    from puxle.pddls.fusion.api import generate_benchmark_with_varying_depth, FusionParams
