...
class Cube3State(State):
    ...
    def __str__(self):
        return f"Cube3State(colors={self.colors})"


class Cube3(Environment):
    moves: List[str] = ["%s%i" % (f, n) for f in ['U', 'D', 'L', 'R', 'B', 'F'] for n in [-1, 1]]
    str_moves: List[str] = ["%s%s" % (f, n) for f in ['U', 'D', 'L', 'R', 'B', 'F'] for n in ["'", ""]]
    moves_rev: List[str] = ["%s%i" % (f, n) for f in ['U', 'D', 'L', 'R', 'B', 'F'] for n in [1, -1]]
    str_moves_rev: List[str] = ["%s%s" % (f, n) for f in ['U', 'D', 'L', 'R', 'B', 'F'] for n in ["", "'"]]
